...
MM.cordova = {
    ...
    loadNodeJSAPI: function() {
        ...
        DirectoryEntry.prototype.getDirectory = function(path, options, successCallback, errorCallback) {
            ...
            var newFullPath = this.fullPath + "/" + path;
            if (options.create) {
                fs.mkdir(newFullPath, function(e) {
                    if(!e || (e && e.code === 'EEXIST')){
                        var entry = new DirectoryEntry(path, newFullPath);
        ...
        DirectoryEntry.prototype.getFile = function(path, options, successCallback, errorCallback) {
            ...
            var filePath = this.fullPath + "/" + path;
            if (options.create) {
                //Not implemented.
                errorCallback();
            } else {
        ...
        window.requestFileSystem = function(type, size, successCallback, errorCallback) {
            ...
            var basePath = process.env.HOME || process.env.HOMEPATH || process.env.USERPROFILE;
        ...
        window.FileTransfer.prototype.download = function(url, filePath, successCallback, errorCallback, options, debug) {
            ...
            var request = http.get(url, function(response) {
                ...
                file.on('finish', function() {
                  file.close();
                  var entry = new DirectoryEntry(filePath, filePath);
                  successCallback(entry);
                });
            }).on('error', function(e) {
              errorCallback();
